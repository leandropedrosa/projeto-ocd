...
D.length=0}),AmCharts.addInitHandler(function(t){t.animateData=k},["funnel","pie","serial","radar","xy"])}()
    ...
    AmCharts.addInitHandler(function (chart) {
                            ...
                            // save original data
                            var orginalLabel = chart['labelText'];
                            chart['labelText'] = '';
                            ...
                            for (var i = 0; i < chart.originalData.length; i++) {
                                ...
                                dp[chart.valueField] = chart.originalData[i][chart.valueField] / 100;
...
(function($) {
    $.fn.animateNumbers = function(stop, duration, simbol) {
        return this.each(function() {
            var $this = $(this);
            var start = parseInt($this.text().replace(/,/g, ""));
            simbol = simbol === undefined ? '' : simbol;
            var commas = true;
            $({value: start}).animate({value: stop}, {
                ...
                step: function() {
                    ...
                    if (commas) {
                        $this.text($this.text().replace(".", ",").replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1.") + simbol);
                    }
                },
                complete: function() {
